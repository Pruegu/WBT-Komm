...
function render() {
  ...
  root.innerHTML = `
    <header class="site-header">
      ...
      <div class="header-actions"><a href="#main">Zum Inhalt</a><span>Seite vorlesen</span><span>Suche</span></div>
      <nav aria-label="WBT Module">
        ${modules.map((name, index) => `<button class="nav-item ${step === index ? 'active' : ''}" data-step="${index}">${name}</button>`).join('')}
      </nav>
    </header>
    <main id="main">
      <div class="breadcrumb">Stadt Zürich / Kommunikation / Web Based Training</div>
  ...
  return `
    <section class="hero">
      <div class="hero-copy">
        ...
        <h1>KI in der Kommunikation</h1>
        <p class="subtitle">Web Based Training der Stadt Zürich</p>
